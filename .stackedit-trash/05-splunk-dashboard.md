...
![Namespace Dashboard](./images/splunk-namespace-dashboard.png "Namespace Dashboard")
...
| timechart avg(Avg) as CPU span=1m by pod_name</query>
  ...
  <row>
    <panel>
      ...
      <chart>
        <search>
          <query>| mstats avg(_value) as "Avg" where index=eks_metrics AND metric_name="kube.pod.network.tx_bytes" AND pod-namespace="splunk-connect" span=1m by pod-name
| eval mb = round(Avg / 1024 / 1024, 3)
| timechart avg(mb) as MBytes span=1m by pod-name</query>
    ...
    <panel>
      ...
      <chart>
        <search>
          <query>| mstats avg(_value) as "Avg" where index=eks_metrics AND metric_name="kube.pod.network.rx_bytes" AND pod-namespace="splunk-connect" span=1m by pod-name
| eval mb = round(Avg / 1024 / 1024, 3)
| timechart avg(mb) as MBytes span=1m by pod-name</query>
          <earliest>-60m@m</earliest>
          <latest>now</latest>
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3ODU5Njk3NDNdfQ==
-->