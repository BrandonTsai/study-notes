...
How to remove value in array in O(1)
-------

Notice that the time complexity of `list.remove()` and `list.index()` method are O(N)


How to get random item from array.
-------

You can not use Set to handle data because `random.sample(population, k, *, counts=None)` the population must be a sequence since version 3.9. Instances of set are no longer supported. The set must first be converted to a list or tuple, preferably in a deterministic order so that the sample is reproducible.
...
Approach 1: maintain self.HashMap
----------
